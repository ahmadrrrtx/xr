...
export const xrMetrics = {
  ...
  // ── Phase 05 · chat streaming observability ───────────────────────────────
  chatRequests: register(new Counter("xr_chat_requests_total", "Chat stream requests by outcome (started|completed|error|cancelled).")),
  chatStreamStarted: register(new Counter("xr_chat_stream_started_total", "Chat SSE streams opened (immediate acknowledgement sent).")),
  chatStreamCompleted: register(new Counter("xr_chat_stream_completed_total", "Chat streams that reached a terminal done event.")),
  chatStreamError: register(new Counter("xr_chat_stream_error_total", "Chat streams that ended in a truthful error event.")),
  chatStreamCancelled: register(new Counter("xr_chat_stream_cancelled_total", "Chat streams cancelled by the client mid-generation.")),
  chatFallbacks: register(new Counter("xr_chat_fallback_total", "Chat generations that used a fallback provider.")),
  chatToolCalls: register(new Counter("xr_chat_tool_calls_total", "Tool calls executed during chat generations by tool/outcome.")),
  chatTtft: register(new Histogram("xr_chat_ttft_ms", "Chat time-to-first-token (ms) by provider/model.")),
