...
export const CATALOG: CatalogEntry[] = [
  ...
  {
    name: "context",
    description: "inspect and control what XR knows",
    usage: "xr context [list|inspect|explain|pending|approve|revoke|correct|export]",
    group: "context",
    glyph: "memory",
    aliases: ["ctx", "knowledge"],
    needsKernel: true,
    examples: [
      { cmd: "xr context", description: "what XR knows, and under what consent" },
      { cmd: "xr context inspect <id>", description: "full provenance for one item" },
      { cmd: "xr context explain", description: "how trust and authority work" },
      { cmd: "xr context revoke <id>", description: "withdraw consent" },
    ],
    related: ["memory", "workspace", "session"],
    topics: ["memory", "context", "knowledge", "privacy"],
    subcommands: [
      { name: "list", description: "browse context items" },
      { name: "inspect", description: "provenance, trust, consent, freshness" },
      { name: "explain", description: "tier and authority reference" },
      { name: "pending", description: "items awaiting your decision" },
      { name: "legacy", description: "pre-4.5 entries with unknown consent" },
      { name: "approve", description: "grant consent" },
      { name: "revoke", description: "withdraw consent (keeps the record)" },
      { name: "correct", description: "replace, preserving lineage" },
      { name: "export", description: "export everything XR holds" },
      { name: "prune", description: "delete expired items" },
    ],
  },
  {
    name: "config",
  ...
  {
    name: "business",
    description: "personal and business operating layer — journeys, outcomes, approvals, AI workers, evidence",
    ...
    needsKernel: true,
    examples: [
      { cmd: "xr business status", description: "operating layer status and configuration" },
      { cmd: "xr business journeys", description: "available outcome journeys" },
      { cmd: "xr business approvals", description: "pending human approvals and escalations" },
    ],
    related: ["run", "agents", "enterprise", "evaluate"],
    topics: ["business", "outcomes", "workflow", "approvals", "work"],
    subcommands: [
      { name: "status", description: "operating layer status" },
      { name: "init", description: "initialize the business operating layer" },
      { name: "journeys", description: "outcome journey catalog" },
      { name: "outcomes", description: "recorded outcomes and their evidence" },
      { name: "approvals", description: "approval queue and escalations" },
      { name: "workers", description: "AI worker contracts and authority" },
      { name: "artifacts", description: "artifact evidence" },
      { name: "mutations", description: "record mutation history" },
      { name: "privacy", description: "local privacy posture" },
      { name: "audit", description: "business audit trail" },
    ],
  },
  {
    name: "evaluate",
    description: "outcome benchmarks, evidence, certification, compatibility, and regression detection",
    usage: "xr evaluate [run|suites|list|inspect|compare|regressions|export|verify|certify|compatibility|claims|limitations|gaps|reproduce]",
    group: "trust",
    glyph: "shield",
    aliases: ["eval", "benchmark"],
    needsKernel: true,
    examples: [
      { cmd: "xr evaluate run --offline", description: "run the offline benchmark subset" },
      { cmd: "xr evaluate run --save", description: "run everything and store the result for comparison" },
      { cmd: "xr evaluate compare <base> <candidate>", description: "detect regressions between releases" },
      { cmd: "xr evaluate claims", description: "every public claim mapped to its evidence" },
      { cmd: "xr evaluate limitations", description: "what the benchmarks do NOT measure" },
    ],
    related: ["enterprise", "trust", "capabilities", "doctor"],
    topics: ["evaluation", "benchmark", "evidence", "certification", "compatibility", "regression"],
    subcommands: [
      { name: "run", description: "run benchmark suites and print a scorecard" },
      { name: "suites", description: "list suites, scenarios, versions, and blind spots" },
      { name: "list", description: "stored evaluation runs" },
      { name: "inspect", description: "full scorecard and per-scenario detail for a run" },
      { name: "compare", description: "regression comparison between two runs" },
      { name: "regressions", description: "compare the two most recent runs" },
      { name: "export", description: "hash-verifiable evidence bundle" },
      { name: "verify", description: "recompute result and bundle integrity" },
      { name: "certify", description: "evidence-backed certification of a provider/capability/workflow/profile/runtime" },
      { name: "compatibility", description: "public API, CLI, and data schema contract checks" },
      { name: "claims", description: "claim to evidence matrix" },
      { name: "limitations", description: "published benchmark blind spots" },
      { name: "gaps", description: "gaps discovered by evaluation, with owners" },
      { name: "reproduce", description: "re-run a scenario to test determinism" },
    ],
  },
  {
    name: "enterprise",
