...
/**
 * Phase 4 · T1 — the bundled seccomp blocklist for bubblewrap
 * (generated by scripts/gen-seccomp.ts; blocked syscalls documented in
 * docs/security/SECURITY_MODEL.md). Applied as defense-in-depth INSIDE the
 * namespace boundary: a process in a user namespace holds namespace-local
 * CAP_SYS_ADMIN, so the seccomp filter additionally denies mount/pivot_root/
 * reboot/kexec/ptrace/bpf/keyring/process_vm_* etc.
 */
function seccompPolicyPath(): string | null {
  const candidates = [
    // src/runtime/trust/environment/namespace.ts → repo root/assets/seccomp
    join(import.meta.dir, "..", "..", "..", "..", "assets", "seccomp", `${process.arch}-blocklist.bpf`),
    join(process.cwd(), "assets", "seccomp", `${process.arch}-blocklist.bpf`),
  ];
...
export class NamespaceSandboxBackend implements EnvironmentBackend {
  ...
  async run(exec: EnvironmentExecutable, ctx: BackendRunContext): Promise<EnvironmentRunResult> {
    ...
    // Phase 4 · T1 — seccomp: open the bundled blocklist and hand it to
    // bubblewrap as fd 3 (`--seccomp 3`). Defense-in-depth inside the
    // namespace boundary; if the file cannot be opened the sandbox still
    // runs (namespaces remain the primary boundary) — this is a deliberate
    // hardening, not the boundary itself.
    let seccompFd: number | null = null;
    const seccompPath = seccompPolicyPath();
    if (this.mechanism === "bwrap" && seccompPath) {
      try {
    ...
    const sandboxArgv =
      this.mechanism === "bwrap"
        ? this.buildBwrap(exec, ctx, writable, envInject, seccompFd !== null)
    ...
    const raw = await runChild({
      ...
      stdin: exec.stdin,
      ...(seccompFd !== null ? { extraFds: [seccompFd] } : {}),
    }).finally(() => {
      if (seccompFd !== null) {
        try {
          closeSync(seccompFd);
  ...
  private buildBwrap(
    ...
    // Phase 4 · T1 — seccomp blocklist (fd 3), applied before exec.
    if (seccomp) a.push("--seccomp", "3");
