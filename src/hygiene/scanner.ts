/**
 * XR System Hygiene Scanner  (was: "XR Shield Service v2")
 * Local-first host hygiene: process, startup, miner and privacy scanning.
 *
 * ── Phase 5 renaming (F-07 · ADR-0027) ──────────────────────────────────────
 *
 * This module was called "XR Shield" and lived at `src/security/shield.ts`.
 * The name was wrong in the way that matters most for a security product: it
 * pointed users at the wrong thing. THIS FILE IS A SCANNER. It inspects the
 * host, reports what it finds, and manages quarantine state. It sits on NO
 * execution path and it enforces NOTHING — an agent action does not pass
 * through this code on its way to running.
 *
 * The component that actually stops an action is a different, distributed
 * ensemble: capability policy evaluation, the action guard, the trust lattice
 * and its placement decision, consent/approvals, egress control, and the
 * hash-chained + Ed25519-signed audit evidence. That ensemble had no name,
 * while this scanner had the protective-sounding one. A reader who trusted the
 * naming would have concluded that turning this scanner off removed their
 * protection, or that leaving it on provided some — both false.
 *
 * So Phase 5 swaps the labels to match reality:
 *   · this scanner        → System Hygiene   (`src/hygiene/`, `xr hygiene`)
 *   · the enforcement path → XR Shield        (`src/xr-shield/`, ADR-0027)
 *
 * No enforcement code was added or removed by that rename; it is a naming
 * correction, which is why it was safe to do mid-stream. `xr shield` remains
 * as a deprecated alias for one release, and `src/security/shield.ts` remains
 * as a re-export shim for out-of-tree importers.
 *
...
/**
 * Canonical Phase 5 name for this service (F-07 · ADR-0027).
 *
 * `XRShieldService` remains exported and is still the class's runtime name, so
 * nothing in-tree or out-of-tree breaks at the rename. New code should use
 * `SystemHygieneScanner`, which says what the class does: it scans the host
 * for hygiene problems. It does not shield anything.
 */
export { XRShieldService as SystemHygieneScanner };

/** Canonical Phase 5 alias for the scanner's finding type. */
export type { ShieldThreat as HygieneFinding };
