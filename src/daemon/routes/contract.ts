...
import {
  ...
  FilesWriteRequest,
  FilesWriteResponse,
  FilesHunksRevertRequest,
  FilesHunksRevertResponse,
  UiStateGetQuery,
  UiStateGetResponse,
  UiStatePatchRequest,
  ...
  TerminalRunRequest,
  TerminalRunEvent,
  TerminalPtyOpenRequest,
  TerminalPtyEvent,
  TerminalPtyInputRequest,
  TerminalPtyInputResponse,
  TerminalPtyResizeRequest,
  TerminalPtyResizeResponse,
  TerminalPtyListResponse,
  TerminalPtyCloseResponse,
...
export const API_CONTRACT: Record<string, ApiOperationMeta> = {
  ...
  "approvals.list": {
    summary: "Durable pending approvals across all surfaces (workspace store).",
    tag: "approvals",
    stability: "stable",
    response: ApprovalsListResponse,
  },
  "approvals.decide": {
    summary: "Decide a durable approval request (approve/deny). TTL default-deny applies.",
    tag: "approvals",
    stability: "stable",
    template: "/api/approvals/{approvalId}/decision",
    pathParams: [{ name: "approvalId", description: "Durable approval record id (e.g. ap_1a2b3c4d)." }],
  ...
  "files.hunks.revert": {
    summary: "Hunk-level REJECT over the engine's own diff: reverse-apply the chosen hunks of a file's working-tree diff via git, after ONE human approval whose preview shows exactly those hunks. Stale ids and changed files are refused (409), never guessed.",
    tag: "workspace",
    stability: "experimental",
    request: FilesHunksRevertRequest,
    response: FilesHunksRevertResponse,
  },
  "state.ui.get": {
    summary: "Desktop UI state for this workspace (layout, tabs, drafts, last area) — opaque JSON the renderer owns, kept durable by the engine.",
    tag: "workspace",
    stability: "experimental",
    request: UiStateGetQuery,
    response: UiStateGetResponse,
  },
  "state.ui.patch": {
    summary: "Upsert/delete desktop UI-state keys (null deletes). Budgeted: ≤ 256 KB per value, ≤ 256 keys per workspace, key shape enforced; all-or-nothing.",
    tag: "workspace",
    stability: "experimental",
    request: UiStatePatchRequest,
    response: UiStatePatchResponse,
  },
  "terminal.run": {
    summary: "Run ONE shell command in the project root — deterministic policy check, durable approval, output streamed as SSE (line-based command runner, NOT a PTY; see terminal.pty.open for the real one).",
    tag: "workspace",
    stability: "experimental",
  ...
  "terminal.pty.open": {
    summary: "Open a REAL interactive terminal (pseudo-terminal: openpty / ConPTY) in the project root — ONE durable high-tier approval per session (keystrokes are not policy-inspected, and the preview says so), cwd scope-enforced, output streamed as SSE, shell killed on disconnect.",
    tag: "workspace",
    stability: "experimental",
    request: TerminalPtyOpenRequest,
    response: TerminalPtyEvent,
    sse: true,
  },
  "terminal.pty.input": {
    summary: "Write keystrokes to an open terminal session (≤ 64 KB per message).",
    tag: "workspace",
    stability: "experimental",
    template: "/api/terminal/pty/{sessionId}/input",
    pathParams: [{ name: "sessionId", description: "Session id from the open stream's status=open event." }],
    request: TerminalPtyInputRequest,
    response: TerminalPtyInputResponse,
  },
  "terminal.pty.resize": {
    summary: "Resize an open terminal session (the child sees the new size).",
    tag: "workspace",
    stability: "experimental",
    template: "/api/terminal/pty/{sessionId}/resize",
    pathParams: [{ name: "sessionId", description: "Session id from the open stream's status=open event." }],
    request: TerminalPtyResizeRequest,
    response: TerminalPtyResizeResponse,
  },
  "terminal.pty.close": {
    summary: "End a terminal session the way a closing window does: SIGHUP, then SIGKILL after a 2 s grace (Windows: close console + terminate).",
    tag: "workspace",
    stability: "experimental",
    template: "/api/terminal/pty/{sessionId}",
    pathParams: [{ name: "sessionId", description: "Session id from the open stream's status=open event." }],
    response: TerminalPtyCloseResponse,
  },
  "terminal.pty.list": {
    summary: "Live terminal sessions owned by this daemon and the per-daemon cap.",
    tag: "workspace",
    stability: "experimental",
    response: TerminalPtyListResponse,
  },
