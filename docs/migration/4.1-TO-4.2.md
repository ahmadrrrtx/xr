...
## Phase 8 addendum — `XR_MCP_ALLOW_UNISOLATED` has been removed

The `XR_MCP_ALLOW_UNISOLATED=1` environment variable described above **no
longer exists**. It was process-wide, anonymous, unsigned and unrevocable: one
variable relaxed kernel isolation for every high-risk MCP server at once, and
nothing recorded who accepted the risk.

Its replacement is a **signed, per-server isolation grant** inside the MCP
allowlist:

```bash
# Before (removed):
export XR_MCP_ALLOW_UNISOLATED=1

# After — explicit, attributable, revocable, and scoped to ONE server:
xr mcp allow <server-id> --allow-unisolated
```

The grant is stored as `isolation: "granted-unisolated-by:<keyId>"` inside the
ed25519-signed allowlist payload, is visible in `xr mcp allowlist-status`, and
is revoked with `xr mcp revoke <server-id>`.

Existing v1 allowlists keep working for one release and are read with every
server at `isolation: "required"` — the safe interpretation. Upgrade with:

```bash
xr mcp re-sign
```

As before, **hardened mode refuses the escape entirely**; the grant only has
effect when hardened mode is explicitly off.
