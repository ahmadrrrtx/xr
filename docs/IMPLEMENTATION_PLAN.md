...
| R-2 | `as any` tightening at provider/routing/config seams (A-6) — no blanket refactor | P2 | hotspot list ≤ 40 sites | ☐ |
...
| R-5 | Onboarding capability detection surfacing (A-12) | P2 | doctor → onboarding hand-off | ☐ |
...
| R-8 | Test hygiene: suite leaves ~220 MB temp dirs on /tmp per run (spurious SQLITE_FULL on full tmpfs); `test/capabilities/provenance-graph.test.ts` "graph is bounded" takes ~4.04 s alone (80% of bun's 5 s default timeout) and flakes under parallel load (observed 5,021 ms once). Fix: route temp dirs under a suite-owned root + per-test cleanup, and either speed up the bounded-graph test (batched inserts) or give it an explicit larger timeout | P2 | not a functional regression — passes deterministically in isolation (3/3) | ☐ |
...
| 8 | **First-run** | ☑ | S-1 fresh-Linux-home journey end-to-end (doctor → onboarding → provider → real task → verify-log → serve → uninstall); friction F1 (P3) / F2 (P2) / F4 recorded + documented; `scripts/golden-path.ts` regression green |
