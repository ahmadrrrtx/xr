...
| **3.x** | Pre-rebaseline published line | `3.0.0`–`3.0.3`, `3.1.5` — **all five versions deprecated on npm (2026-09-08)** | `v3.0.0` | retired — the deprecation message points to the 1.x line |
...
| **1.0.0** | Deliberate semver rebaseline of 7.1.0 ("Truth") | **`1.0.0-beta.1` on npm `beta`; `1.0.0` stable on npm `latest` (both 2026-09-08)** | `v1.0.0-beta.1`, `v1.0.0` | **current** — stable `1.0.0` is the production line; pre-releases continue on the `beta` dist-tag |

## Dist-tags (operator, not CI)

Current state (2026-09-08): `latest` → `1.0.0`, `beta` → `1.0.0-beta.1`. The entire 3.x line
is deprecated on npm with a message pointing here and to the 1.x line. Until stable 1.0.0,
keeping `3.1.5` on `latest` was **intentional**: auto-repointing `latest` to a
`1.0.0-beta.*` would ship a prerelease to every `npm i @rrrtx/xr` user. The repoint happened as
part of the stable publish, and the tag ⇔ npm invariant
(`bun run scripts/tag-npm-invariant.ts`) is checked in CI so the tags and dist-tags cannot
