...
| 21 | **On Windows CI, the child-process durable-approval tests (kill -9, cross-process) are skipped.** On Windows runners, stdout pipes of `Bun.spawn` children hang without closing: the stream reads (and the cross-process `for await`) never settle even after a watchdog kills the child, so the parity lane stalled 30 min with zero test failures — exit 124, "dies alone" (2026-09-07, Windows full-parity, runs 34152171256 and 34156612385). The in-process durability coverage (TTL, persistence, restart re-attach, sweep, first-writer-wins) runs on every platform and the full spawn matrix runs on the Linux reference lane and macOS. Revisit when Bun's Windows pipe EOF is fixed. | trust | ci | 2026-12-31 | [approvals-durable.test.ts](../../../test/phase2/approvals-durable.test.ts) (`skipIf` win32) |
