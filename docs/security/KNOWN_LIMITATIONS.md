...
| 21 | **On Windows CI, the child-process durable-approval tests (kill -9, cross-process) are skipped; the in-process suite now ends with zero live timers.** On Windows runners this file died at the segment cap with zero output and zero test failures (exit 124, "dies alone" — 2026-09-07, runs 34152171256 / 34156612385 / job 101865773607): the process never finished, so results were never flushed. Root-cause analysis: it is the only file in its suite segment that creates `setInterval` (ApprovalStore cross-process pollers); a record left pending kept a 150ms poller + 30s TTL timer live and the bun 1.3.14 process would not exit on Windows. Every test now calls `approvals.dispose()` (zero live timers at file end). The spawn tests' stdout-pipe behavior on Windows remains UNVERIFIED (the runs died before it could be observed) and they stay `skipIf(win32)` until a passing Windows run proves them; the full spawn matrix runs on the Linux reference lane and macOS. | trust | ci | 2026-12-31 | [approvals-durable.test.ts](../../../test/phase2/approvals-durable.test.ts) (`skipIf` win32 + dispose) |
