...
| 18 | **`docs/CONSTITUTION.md` is a RECONSTRUCTION, not the original text.** The 30 Articles are cited ~300 times across the codebase and enforced by real gates, but the original prose was never committed. The published document was rebuilt from those citations: clauses marked **[V]** are verbatim quotes found in citing files; **[R]** clauses are reconstructed from how the Article is cited and enforced. Articles **XIII, XVII, XXV, XXVI** have zero citations and are recorded as *unattested* rather than invented. `claim-lint` enforces Article-level citations only — clause numbers (`IV.4`, `XII · Rule 4`) are cited with more precision than the reconstruction can honestly vouch for. | governance | docs | — | [CONSTITUTION.md](../CONSTITUTION.md) preamble; Phase 5 |
| 19 | **Satellite packages are not published yet.** `@rrrtx/xr-enterprise` and `@rrrtx/business-os` exist as complete, tested packages under `satellites/` with their own manifests, but no npm publish or standalone repo push has happened. Until then the install lines in the relocation notices and migration guide describe the intended state, not a fetchable package. | distribution | release | on first satellite publish | [ADR-0028](../adr/0028-satellite-extraction.md); [PHASE-5-SATELLITES](../migration/PHASE-5-SATELLITES.md) |
| 20 | **The 136,000-LOC tree ceiling is a no-regrowth guard, not the plan's ≤110k target.** Phase 5 measured 154,426 → 131,050 LOC and set the gate at 135,000; Phase 7 raised it once, deliberately, to 136,000 for the ~1,200 LOC memory policy layer (ACL, provenance, arbitration, consolidation, forget/export — core surface, not satellite material), with the reason recorded in `scripts/size-gate.ts`. Reaching 110k would require extracting `src/research` and `src/repo` (11 versioned API operations and 11 agent tools — vetoed as live capability) or cutting genuine runtime. The gate holds the line actually achieved; 110k remains a direction of travel, not a passing grade anyone can currently award. | product | architecture | — | [loc-census](../historical/phase-5/loc-census.md); [ADR-0028](../adr/0028-satellite-extraction.md) |
| 21 | **On Windows CI, `test/phase2/approvals-durable.test.ts` runs a staged diagnostic probe instead of the real suite — the Windows hang is still unrooted.** On Windows runners this file dies at the segment cap with zero output and zero test failures (exit 124, "dies alone" — 2026-09-07, runs 34152171256 / 34156612385, jobs 101865773607 / 101873414375): the process never finishes, so per-file buffered output is lost. Three fixes were tried and each left the identical signature: child-stdout watchdogs, `skipIf(win32)` on the spawn describes, and `dispose()` for zero live timers at file end (the live-timer theory is now disproven — the hang predates all three). Ruled out by comparison with files that pass the same Windows lane: `new Store`/bun:sqlite + `mkdtempSync` (test/context/, 21 files), open sqlite handles at file end (context files never close), line endings (LF everywhere). Since per-file stdout is lost on the kill, the file now runs staged probes on win32 only (D1 bare → D2 Store open/audit/close → D3 approval-store module load via dynamic import → D5 bare timer → D4 full request/TTL flow), each writing on-disk `xr-dbg-*.txt` markers that survive the kill; the workflow step "Hang diagnostics (win32)" in `cross-platform.yml` dumps them on failure — the stage with START but no END names the culprit. The full real suite runs on the Linux reference lane and macOS (both green). Remove the probe branch + workflow step when the Windows lane passes. | trust | ci | 2026-12-31 | [approvals-durable.test.ts](../../../test/phase2/approvals-durable.test.ts) (win32 probe branch) |
...
*Updated 2026-09-05 (Phase 5): rows #18–#20 added — the Constitution's
reconstructed provenance, unpublished satellites, and the re-baselined LOC
ceiling. Each is a claim Phase 5 could have made quietly and did not.*
