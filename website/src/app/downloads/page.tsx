      ...
      <section>
        <div className="mx-auto max-w-5xl px-6">
          <div className="card p-8 text-center">
            <div className="text-sm text-slate-400">Quick install</div>
            <div className="mt-4 flex justify-center">
              <div className="w-full max-w-md">
                <InstallCmd />
              </div>
            </div>
            <div className="mt-6 text-xs text-slate-500">Requires Node 20+ · Full install ≈ 40MB</div>
          </div>
          ...
          <div className="mt-5 grid md:grid-cols-3 gap-5">
            {platforms.map((p) => (
              <a key={p.label} href={p.href} className="card p-6 group flex flex-col">
                <p.icon className="h-7 w-7 text-slate-300" />
                <div className="mt-4 text-lg font-semibold text-white">{p.label}</div>
                <div className="text-xs text-slate-500 mt-1">v{p.version}</div>
                <div className="text-sm text-slate-400 mt-2 flex-1">{p.note}</div>
          ...
          <div className="mt-5 grid md:grid-cols-3 lg:grid-cols-5 gap-4">
            {editors.map((e) => (
              <a key={e.label} href={e.href} className="card p-5 group">
                <e.icon className="h-5 w-5 text-slate-400" />
...
function Code({ code, comment }: { code: string; comment: string }) {
  return (
    <div className="rounded-xl border border-slate-700/40 bg-black/40 px-4 py-3 flex items-center gap-3">
      <span className="text-cyan-400">$</span>
      <code className="text-slate-100 flex-1">{code}</code>
