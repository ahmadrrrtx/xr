      ...
      <section className="pb-24">
        <div className="mx-auto max-w-5xl px-6 space-y-4">
          {research.map((r) => (
            ...
            >
              <div className="h-10 w-10 rounded-xl flex items-center justify-center bg-gradient-to-br from-sky-500/20 to-cyan-500/10 border border-slate-700/40 shrink-0">
                <FileText className="h-5 w-5 text-sky-300" />
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2 text-[11px] text-slate-500">
                  <span className="px-1.5 py-0.5 rounded bg-white/5 border border-slate-800/60">{r.tag}</span>
                  <span>{r.year}</span>
                </div>
                <h3 className="mt-1.5 text-white font-semibold group-hover:text-cyan-200 transition-colors">
                  {r.title}
                </h3>
                <p className="text-sm text-slate-500 mt-1">{r.authors}</p>
              </div>
