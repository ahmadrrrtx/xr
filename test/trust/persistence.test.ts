...
describe("XR 4.2 trust metadata durability", () => {
  test("trust block (tier, decision, verification, cleanup) round-trips through the repo", async () => {
    if (SKIP_SANDBOX) {
      console.warn("Skipping trust durability test: bwrap not available or CI environment");
      return;
    }
    ...
    try {
      ...
      console.warn(`Trust durability sandbox test failed (CI env may lack bwrap): ${(e as Error).message}`);
    }
