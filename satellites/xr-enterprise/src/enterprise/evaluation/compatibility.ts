...
import { CORE_VERSION, PLUGIN_API_VERSION } from "@xr/core/core/version.ts";
import { CAPABILITY_DESCRIPTOR_SCHEMA_VERSION } from "@xr/core/platform/capabilities/types.ts";
import { WORKFLOW_DEFINITION_SCHEMA_VERSION } from "@xr/core/execution/workflow/types.ts";
...
export async function checkPublicApi(baseline: ContractBaseline = XR_7_0_CONTRACT_BASELINE): Promise<CompatibilityCheck[]> {
  const loaders: Record<string, () => Promise<Record<string, unknown>>> = {
    execution: () => import("@xr/core/execution/index.ts"),
    trust: () => import("@xr/core/runtime/trust/index.ts"),
    context: () => import("@xr/core/context/index.ts"),
    workflow: () => import("@xr/core/execution/workflow/index.ts"),
    capabilities: () => import("@xr/core/platform/capabilities/index.ts"),
    intelligence: () => import("@xr/core/intelligence/index.ts"),
    environment: () => import("@xr/core/platform/environment/index.ts"),
...
export async function checkCli(baseline: ContractBaseline = XR_7_0_CONTRACT_BASELINE): Promise<CompatibilityCheck[]> {
  try {
    const catalog = (await import("@xr/core/cli/catalog.ts")) as Record<string, unknown>;
...
export async function checkWorkflowLegacyDefinitions(): Promise<CompatibilityCheck[]> {
  try {
    const nodes = await import("@xr/core/execution/workflow/nodes.ts");
    const versioning = await import("@xr/core/execution/workflow/versioning.ts");
    const types = await import("@xr/core/execution/workflow/types.ts");
