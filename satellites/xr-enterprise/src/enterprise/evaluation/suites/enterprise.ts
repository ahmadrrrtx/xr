...
import { CORE_VERSION } from "@xr/core/core/version.ts";
import {
...
const contractsAreDiscoverable: ScenarioDefinition = {
  ...
  run: async (ctx) => {
    ...
    const barrels: { name: string; load: () => Promise<Record<string, unknown>> }[] = [
      { name: "execution", load: () => import("@xr/core/execution/index.ts") },
      { name: "trust", load: () => import("@xr/core/runtime/trust/index.ts") },
      { name: "context", load: () => import("@xr/core/context/index.ts") },
      { name: "workflow", load: () => import("@xr/core/execution/workflow/index.ts") },
      { name: "capabilities", load: () => import("@xr/core/platform/capabilities/index.ts") },
      { name: "intelligence", load: () => import("@xr/core/intelligence/index.ts") },
      { name: "environment", load: () => import("@xr/core/platform/environment/index.ts") },
...
const refusalsAreUnderstandable: ScenarioDefinition = {
  ...
  run: async (ctx) => {
    const { classifyRisk } = await import("@xr/core/runtime/trust/classify.ts");
    const { decidePlacement } = await import("@xr/core/runtime/trust/policy.ts");
    const { admitContextWrite } = await import("@xr/core/context/poison.ts");
...
const approvalsExplainConsequence: ScenarioDefinition = {
  ...
  run: async (ctx) => {
    const { assessEnvironmentAction } = await import("@xr/core/platform/environment/classify.ts");
    const { EnvironmentActionRequestSchema } = await import("@xr/core/platform/environment/types.ts");
