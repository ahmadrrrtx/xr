 ...
 *   - src/platform/environment/classify.ts(action assessment, reversibility, approval)
 *   - src/enterprise/deployment/profiles.ts (profile capability matrix, portability)
 */

import { CORE_VERSION, PKG, versionInfo } from "@xr/core/core/version.ts";
import { IntelligenceRouter } from "@xr/core/intelligence/router.ts";
import type { IntelligenceCatalog } from "@xr/core/intelligence/catalog.ts";
import { assessEnvironmentAction } from "@xr/core/platform/environment/classify.ts";
import { EnvironmentActionRequestSchema } from "@xr/core/platform/environment/types.ts";
import {
...
const fixtureIsolation: ScenarioDefinition = {
  ...
  set: "independent",
  determinism: "deterministic",
  contracts: ["src/enterprise/evaluation/fixtures.ts#FixtureWorkspace"],
  profiles: [],
  offlineCapable: true,
...
async function catalogWithoutKeyProbe(): Promise<IntelligenceCatalog> {
  const { PRESETS } = await import("@xr/core/providers/presets.ts");
  const { buildCatalog } = await import("@xr/core/intelligence/catalog.ts");
...
const routingIsExplainable: ScenarioDefinition = {
  ...
  run: async (ctx) => {
    const { routingDecisionToRecord } = await import("@xr/core/intelligence/router.ts");
    const catalog = await catalogWithoutKeyProbe();
    const router = new IntelligenceRouter({ catalog });
...
const profilePortability: ScenarioDefinition = {
  ...
  determinism: "deterministic",
  contracts: [
    "src/enterprise/deployment/profiles.ts#listDeploymentProfiles",
    "src/enterprise/deployment/profiles.ts#validateProfileCompatibility",
    "src/enterprise/deployment/profiles.ts#isCapabilityAvailable",
  ],
  profiles: [],
  offlineCapable: true,
  allowedEffects: NO_EXTERNAL_EFFECTS,
...
const cloudProfileHonesty: ScenarioDefinition = {
  ...
  set: "validation",
  determinism: "deterministic",
  contracts: ["src/enterprise/deployment/profiles.ts#validateProfileCompatibility"],
  profiles: [],
  offlineCapable: true,
  allowedEffects: NO_EXTERNAL_EFFECTS,
  budget: BUDGET,
